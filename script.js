    ...
    Object.keys(translations.sections).forEach(sectionKey => {
        ...
        if (section) {
            ...
            if (sectionKey === 'experience') {
                const contentElement = section.querySelector('div') || section.appendChild(document.createElement('div'));
                let html = '';
                translations.sections[sectionKey].content[lang].forEach(job => {
                    html += `<h4>${job.title}</h4><ul>`;
                ...
                if (contentElement) {
                    contentElement.innerHTML = translations.sections[sectionKey].content[lang].replace(/\n/g, '<br>');
                }
            } else {
                const contentElement = section.querySelector('p, ul');
                if (contentElement) {
                    if (Array.isArray(translations.sections[sectionKey].content[lang])) {
                        contentElement.innerHTML = translations.sections[sectionKey].content[lang].map(item => `<li>${item}</li>`).join('');
                    } else {
